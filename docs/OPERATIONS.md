...
5. Generate. The model sees the approved photos and invents the layout; the
   preview footer names which model produced the draft, which is how you
   compare one generator against another. The draft is checked for unsafe
   markup and other hard HTML rules, not for a named page shape. Your job is
