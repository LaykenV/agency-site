...
- **Traction today:** **3 free clients (family / friends). 0 paying clients.** Two of those serve as public testimonials (All About Towing, TB Tree Service).
...
The bespoke template architecture supports a future repositioning to ~$399/mo once 3+ paying clients are signed and 2–3 strong bespoke portfolio pieces exist. Trigger the price raise on the *product launch* (i.e., when bespoke becomes the default offering publicly), not on a client count. Grandfather existing $199 clients forever. The 12-month minimum already protects retention.
...
7. Bespoke design per client (after migration to the new template) — visual ceiling no longer capped by a section registry
...
Full-stack Next.js 16 + Convex application. See `ARCHITECTURE.md` for the technical blueprint.
...
- **Hub ↔ Spoke architecture** — client template sites POST leads and pageview events to this Convex backend. Versioned endpoints (`/api/v1/*`) and unversioned legacy endpoints both supported.

### Outbound marketing pipeline (`/admin/marketing`)

Working automated lead-gen system:
...
4. PageSpeed Insights runs a mobile performance score.
...
- Cold dialing at scale (caused the paralysis — keep as a follow-up tool for warm leads only).
...
- Brand identity: blue-dominant (`hsl(215, 85%, 55%)`) + gold accent (`hsl(43, 74%, 52%)`).
- The bespoke `agency-template` repo for building custom client sites that share standardized WaaS plumbing.

---

## 7. What to Help With Next

Most strategic questions reduce to one of:

1. **Conversion / sales help** — closing the first cold-started client. Drop-in scripts, follow-up cadence, objection handling, partnership pitches.
...
5. **Specific prospect strategy** — high-value or unusual-vertical prospects.

If a question falls outside those five, route it back through the channel framework above before pursuing it. Anything that adds tooling complexity, broad-funnel SEO, content-treadmill social, or outsourced marketing services is almost certainly a distraction at this stage.
