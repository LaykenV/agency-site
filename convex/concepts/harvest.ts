 ...
 *
 * Nothing here decides anything. It produces candidates with source excerpts
 * and parks the concept in `content_review`; `lib/concepts/harvest.ts` holds
 * every rule about what a candidate may become, and the database writes live in
 * `concepts/internal.ts` with the rest of the transactional surface.
 *
 * Failure is partial by design. A page that will not scrape becomes a warning,
 * not a lost harvest; a Firecrawl-wide failure leaves the concept exactly as it
 * was, with manual notes and uploads untouched.
 */
